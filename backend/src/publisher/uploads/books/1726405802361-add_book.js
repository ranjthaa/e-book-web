...
const path = 'C:\\e-book\\web\\e-book\\client\\e-book-web\\backend\\src\\publisher\\uploads\\books\\'
...
const storage = multer.diskStorage({
  ...
  destination: (req, file, cb) => { 
    if(file.fieldname === 'C:\\e-book\\web\\e-book\\client\\e-book-web\\backend\\src\\publisher\\uploads\\cover_image\\') {
        cb(null, 'cover_image//'); 
    }
    if(file.fieldname === 'book') {
       cb(null, 'C:\\e-book\\web\\e-book\\client\\e-book-web\\backend\\src\\publisher\\uploads\\books\\');
    }
    if(file.fieldname === 'demo_file') {
        cb(null, 'C:\\e-book\\web\\e-book\\client\\e-book-web\\backend\\src\\publisher\\uploads\\demo_file\\');
    }
...
const add_book = (req, res) => {
    ...
    upload.fields([{ name: 'cover_image', maxCount: 1 }, { name: 'book', maxCount: 1 }, { name:'demo_file', maxCount: 1}])(req, res, (err) => {
        ...
        if(err) {
            ...
            return res.status(400).send({ status:400, message: 'image data missing'});
        }
